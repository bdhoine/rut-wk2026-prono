...
import type { RecapData, RecapStatLeader, RecapStanding } from "@/lib/data";
...
type Slide =
  | { kind: "intro" }
  | { kind: "matches"; played: number; total: number; goals: number }
  | { kind: "topscorer"; player: string; teamName: string | null; teamIso: string | null; goals: number; moreCount: number }
...
function buildSlides(data: RecapData): Slide[] {
  ...
  if (data.matchesPlayed > 0) slides.push({ kind: "matches", played: data.matchesPlayed, total: data.matchesTotal, goals: data.goalsTotal });
...
// for the #1 finale slide only. Purely decorative — disabled under
...
function SlideContent({ slide }: { slide: Slide }) {
  switch (slide.kind) {
    case "intro":
      return (
        <div className="recap-pop flex flex-col items-center gap-3 text-center">
          <span className="text-6xl">🏆</span>
          <h2 className="font-display text-3xl font-extrabold tracking-tight">Jouw WK Recap</h2>
          <p className="max-w-xs text-sm text-ink-foreground/70">De cijfers, de reeksen en het klassement van Rut Prono WK 2026</p>
        </div>
      );
    case "matches":
      return (
        <div className="recap-pop text-center">
          <span className="recap-bounce text-5xl">⚽</span>
          <p className="mt-4 font-display text-5xl font-extrabold tabular-nums">
            {slide.played}
            <span className="text-2xl text-ink-foreground/50">/{slide.total}</span>
          </p>
          <p className="mt-1 text-sm uppercase tracking-wide text-ink-foreground/70">wedstrijden gespeeld</p>
          <p className="mt-7 font-display text-4xl font-extrabold tabular-nums text-gold">{slide.goals}</p>
    ...
    case "topscorer":
      return (
        <div className="recap-pop text-center">
          <p className="text-xs font-semibold uppercase tracking-widest text-gold">Topschutter</p>
          {slide.teamIso && <FlagIcon iso={slide.teamIso} className="mt-4 text-5xl" />}
          <p className="mt-4 font-display text-3xl font-extrabold">{slide.player}</p>
          {slide.teamName && <p className="text-sm text-ink-foreground/70">{slide.teamName}</p>}
          <p className="mt-5 font-display text-5xl font-extrabold tabular-nums text-gold">
            {slide.goals} <span className="text-lg font-semibold text-ink-foreground/70">goals</span>
          </p>
          {slide.moreCount > 0 && (
            <p className="mt-2 text-xs text-ink-foreground/60">+{slide.moreCount} andere{slide.moreCount > 1 ? "n" : ""} met evenveel goals</p>
          )}
        </div>
      );
    case "champion":
      return (
        <div className="recap-pop text-center">
          <span className="recap-bounce text-5xl">🏆</span>
          <p className="mt-3 text-xs font-semibold uppercase tracking-widest text-gold">Wereldkampioen</p>
          <FlagIcon iso={slide.teamIso} className="mt-3 text-6xl" />
          <p className="mt-4 font-display text-3xl font-extrabold">{slide.teamName}</p>
        </div>
      );
    case "streak":
      return (
        <div className="recap-pop text-center">
          ...
          <p className="mt-3 text-xs font-semibold uppercase tracking-widest text-gold">{slide.title}</p>
          <p className="mt-2 font-display text-6xl font-extrabold tabular-nums">{slide.leader.value}</p>
          <p className="text-sm text-ink-foreground/70">{slide.subtitle}</p>
          <p className="mt-5 font-display text-xl font-bold">{slide.leader.names.join(", ")}</p>
          {slide.leader.moreCount > 0 && <p className="mt-1 text-xs text-ink-foreground/60">+{slide.leader.moreCount} anderen</p>}
        </div>
      );
    case "standing": {
      ...
      if (!slide.finale) {
        return (
          <div className="recap-pop text-center">
            <p className="text-xs font-semibold uppercase tracking-widest text-gold">Klassement</p>
            <p className="mt-1 font-display text-7xl font-extrabold text-gold">#{s.position}</p>
            <p className="mt-4 font-display text-2xl font-bold">{s.names.join(", ")}</p>
            {s.moreCount > 0 && <p className="mt-1 text-xs text-ink-foreground/60">+{s.moreCount} anderen</p>}
            <p className="mt-3 text-sm text-ink-foreground/70">
              {s.total} punten{s.prize > 0 ? ` · ${fmtEuro(s.prize)}` : ""}
            </p>
          </div>
        );
      }
      return (
        <div className="relative flex h-full w-full flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 text-xs font-semibold uppercase tracking-widest text-gold">Nummer één</p>
          <p className="mt-2 font-display text-4xl font-extrabold">{s.names.join(" & ")}</p>
          {s.moreCount > 0 && <p className="mt-1 text-xs text-ink-foreground/70">+{s.moreCount} anderen</p>}
          <p className="mt-4 font-display text-6xl font-extrabold tabular-nums text-gold">{s.total}</p>
          <p className="text-sm text-ink-foreground/70">punten</p>
          {s.prize > 0 && (
