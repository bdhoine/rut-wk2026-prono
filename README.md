...
| `/statistieken` | Top scorers, most goals scored/conceded (unplayed teams hidden behind "Toon alle"), top-10 matches by points and by wrong predictions, popular scorelines, most-picked winner/top scorer (top 5 with a "Toon alle" expander; the top-scorer table shows the player's flag + WK goals). Picked top-scorer names link to their `/topschutter/[slug]` profile |
